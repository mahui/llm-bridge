...
LLM-Bridge is a local AI proxy gateway for personal use that wraps three AI CLI harnesses (Claude Code via Agent SDK, Codex, Antigravity CLI) behind a single OpenAI-compatible API. Each provider's official harness handles its own OAuth/token management — the gateway never touches tokens.
...
- **`reasoning_effort`** (OpenAI-standard request field, low/medium/high/xhigh) is honored by claude (SDK `effort` option; gateway defaults to medium, NOT the SDK's high) and codex (`-c model_reasoning_effort`); ignored by agy (depth is encoded in model variants). UI settings expose it.
- **Codex/Antigravity(agy)** use CLI subprocesses. Prompts are sent via **stdin** (not args) to avoid OS argument length limits. Streaming paths use `stderr=DEVNULL` (an undrained pipe deadlocks the child) and a `finally` block that kills the child on client disconnect — keep both invariants when editing.
...
Agy:     agy -p - --model {display-name}   # Antigravity CLI; plain-text output
```
...
Both read prompts from stdin. Codex outputs line-delimited JSON; agy outputs plain text (models listed dynamically via `agy models`, slug<->display-name map in agy.py).
...
- **provider-engineer** — provider 适配层：子进程三不变量（stderr DEVNULL / finally kill / stdin drain）、SDK/CLI 升级适配、合规红线（禁 token 直连）。gemini CLI 已废弃，Google 现行 harness 是 Antigravity CLI（agy）。
...
- CLI subprocess latency ~3-8s per request (codex/agy)
- Model lists: codex reads `$CODEX_HOME/models_cache.json` (CLI-maintained); claude uses the
  ...
  else hardcoded fallback; agy fully dynamic via `agy models`
